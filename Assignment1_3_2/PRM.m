...
for i = 2:nsamples
    ...
    neighbours = min(k,length(distances));
    for j=1:neighbours
        x_index = index_sorted(j);

        x_sample = samples(:,x_index);

        x_distance = distances_sorted(j);

        if LocalPlanner(x_sample, samples(:,i))
            nedges = nedges + 1;
            edges(nedges,:)=[i,x_index];
            edge_lengths(nedges) = x_distance;
